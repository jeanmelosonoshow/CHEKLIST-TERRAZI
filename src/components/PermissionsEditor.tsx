...
export function PermissionsEditor({ initialRows }: { initialRows: Row[] }) {
  ...
  return <>
    {message && <p className={message.startsWith("Permissões") ? "formSuccess" : "formError"} role="status">{message}</p>}
    <div className="tableWrap permissionsTable"><table><thead><tr><th>Categoria</th>{columns.map((column) => <th key={column.key}>{column.label}</th>)}<th>Ação</th></tr></thead><tbody>{rows.map((row) => <tr key={row.category}><td><strong>{row.category}</strong><small>{row.configured ? "Configurada" : "Sem configuração — acesso negado"}</small></td>{columns.map((column) => <td key={column.key}><label className="permissionToggle"><input type="checkbox" checked={row[column.key]} onChange={() => toggle(row.category, column.key)} /><span>{row[column.key] ? "Sim" : "Não"}</span></label></td>)}<td><button className="primaryButton" disabled={saving === row.category} onClick={() => save(row)}>{saving === row.category ? "Salvando…" : "Salvar"}</button></td></tr>)}</tbody></table></div>
  </>;
}
