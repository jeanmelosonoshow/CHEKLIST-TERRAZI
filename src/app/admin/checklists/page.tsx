...
export default async function AdminChecklistsPage() {
  ...
  return (
    <AppShell user={user} admin>
      <section className="heroRow"><div><p className="eyebrow">ADMINISTRAÇÃO</p><h1>Checklists</h1><p className="muted">Crie e organize os fluxos que serão usados pela equipe.</p></div><ChecklistBuilder sources={sources} /></section>
      ...
      <section className="sectionBlock"><div className="sectionHeading"><div><p className="eyebrow">BIBLIOTECA</p><h2>Todos os checklists</h2></div><span className="sourceInfo">{sources.length} fonte(s) sincronizada(s)</span></div>
        {items.length ? <div className="tableWrap"><table><thead><tr><th>Checklist</th><th>Status</th><th>Campos</th><th>Atualizado</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><small>{item.description || "Sem descrição"}</small></td><td><span className={`status ${item.status}`}>{item.status === "draft" ? "Rascunho" : item.status === "published" ? "Publicado" : "Arquivado"}</span></td><td>{item.fieldCount}</td><td>{new Intl.DateTimeFormat("pt-BR").format(item.updatedAt)}</td></tr>)}</tbody></table></div> : <div className="emptyState"><span>☷</span><h3>Seu primeiro checklist começa aqui</h3><p>Crie um rascunho, adicione perguntas e conecte fontes sincronizadas.</p></div>}
      </section>
    </AppShell>
  );
}
