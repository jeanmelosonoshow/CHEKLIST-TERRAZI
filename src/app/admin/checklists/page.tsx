  ...
  return (
    <AppShell user={user} permissions={permissions} active="management">
      ...
      <section className="sectionBlock"><div className="sectionHeading"><div><p className="eyebrow">BIBLIOTECA</p><h2>Todos os checklists</h2></div><span className="sourceInfo">{sources.length} fonte(s) sincronizada(s)</span></div>
        {items.length ? <div className="tableWrap"><table><thead><tr><th>Checklist</th><th>Status</th><th>Campos</th><th>Atualizado</th>{permissions.editChecklist && <th>Ações</th>}</tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{item.title}</strong><small>{item.description || "Sem descrição"}</small></td><td><span className={`status ${item.status}`}>{item.status === "draft" ? "Rascunho" : item.status === "published" ? "Publicado" : "Arquivado"}</span></td><td>{item.fieldCount}</td><td>{new Intl.DateTimeFormat("pt-BR").format(item.updatedAt)}</td>{permissions.editChecklist && <td>{item.status === "draft" ? <ChecklistEditor checklistId={item.id} sources={sources} /> : <span className="muted">Bloqueado</span>}</td>}</tr>)}</tbody></table></div> : <div className="emptyState"><span>☷</span><h3>Seu primeiro checklist começa aqui</h3><p>Crie um rascunho, adicione perguntas e conecte fontes sincronizadas.</p></div>}
      </section>
    </AppShell>
  );
}
