...
export default async function AdminChecklistsPage() {
  ...
  const [items, sources] = await Promise.all([
    db.select({ id: checklists.id, code: checklists.code, title: checklists.title, description: checklists.description, status: checklists.status, updatedAt: checklists.updatedAt, fieldCount: sql<number>`count(${checklistFields.id})::int` }).from(checklists).leftJoin(checklistFields, eq(checklists.id, checklistFields.checklistId)).groupBy(checklists.id).orderBy(desc(checklists.updatedAt)),
